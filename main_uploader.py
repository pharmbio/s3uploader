...
def setup_logging():
    ...
    logging.basicConfig(
        level=logging.DEBUG,
        format='%(asctime)s %(levelname)-8s %(threadName)s [%(filename)-30s:%(lineno)4d] %(message)s',
        datefmt='%Y-%m-%d %H:%M:%S',
        stream=sys.stdout
    )
...
def main():
    ...
    uploader = S3ImageUploader(db_config=db_config, s3_config=s3_config, max_workers=3, sleep_time=10)
